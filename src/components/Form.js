...
//------------- FOR TESTING START -------------------------------------
  ...
  sendEmail = () => {
    const {theme, usernameFrom, emailFrom, usernameTo, message, emailTo, files} = this.state.values
    const {setStatusEmail, setPreload} = this.props
    const obj = {
      "letter": {
        "subject": theme,
    ...
    setPreload(true)
    apiSend(obj).then(({status, track, theme}) => {
      console.log('status, track, theme => ', status, track, theme)
      setStatusEmail({status, theme, track})
      this.setState({
        disabled: false
      })
    })
  ...
  render(){
    ...
    return(
      <form encType="multipart/form-data" method="post">
        ...
        <button
          type="submit"
          className={this.state.disabled ? "btn btn-secondary disabled" : "btn btn-primary"}
